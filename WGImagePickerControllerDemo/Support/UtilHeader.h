...
#define NAV_BAR_HEIGHT ((PORTRAIT ? (SCREEN_NEW ? (SCREEN_HEIGHT * 0.108 - STATUS_BAR_HEIGHT) : 44) : 32) + STATUS_BAR_HEIGHT) // 导航栏高度(包含状态栏)
#define NAV_BAR_HEIGHT_OFFSET ((SCREEN_NEW && PORTRAIT) ? 24 : 0) // 状态栏高度偏差
#define BOTTOM_SAFE_MARGIN ((SCREEN_NEW && PORTRAIT) ? 34 : 0) // 底部安全间隙
#define TAB_BAR_HEIGHT ((SCREEN_NEW ? (SCREEN_HEIGHT * 0.1034 - BOTTOM_SAFE_MARGIN) : 45) + BOTTOM_SAFE_MARGIN) // 标签栏高度(包含底部安全间隙)
